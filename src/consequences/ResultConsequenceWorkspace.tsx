...
export default function ResultConsequenceWorkspace({ onOpenLiveCard, onOpenPlanner }: ResultConsequenceWorkspaceProps) {
  ...
  function rollback(): void {
    ...
    try {
      const result = rollbackCoreResultConsequences(universe, selectedApplication.id, rollbackReason, profiles, { shows, championships, competitions });
      setUniverse(result.universe);
      setShows(result.shows);
      setChampionships(result.championships);
      setCompetitions(result.competitions);
      ...
      saveMatchEngineUniverse(window.localStorage, { profiles: result.profiles });
      setRollbackReason("");
      setNotice("The selected consequence event was excluded and every later official result was replayed in chronology. The original match calculation remains locked in Match Resolution history.");
    } catch (caught) {
  ...
  function confirmTitle(proposal: ChampionshipConsequenceProposal): void {
    try {
      ...
      setShows(result.shows);
      setChampionships(result.championships);
      setNotice(`${proposal.championshipName} consequence confirmed. Championship activity was rebuilt in official chronology.`);
    } catch (caught) {
  ...
  return <section className="consequence-workspace">
    ...
    <section className="consequence-metrics"><article><span>Results ready</span><strong>{pending.length}</strong></article><article><span>Applied matches</span><strong>{universe.applications.filter((application) => application.status === "Applied" && application.replayStatus !== "Superseded").length}</strong></article><article><span>Angle events</span><strong>{universe.competitiveProfileAdjustments.length}</strong></article><article><span>Worker records</span><strong>{universe.workerRecords.length}</strong></article><article><span>Team records</span><strong>{universe.teamRecords.length}</strong></article><article><span>Title decisions</span><strong>{unresolvedTitles.length}</strong></article><article><span>Competition decisions</span><strong>{unresolvedCompetitions.length}</strong></article><article><span>Future reviews</span><strong>{openConflicts.length}</strong></article></section>
    ...
    {universe.settings.activeTab === "overview" && <div className="consequence-overview">
      ...
      <section className="consequence-applications"><header><div><p className="eyebrow">APPLICATION HISTORY</p><h3>Official chronological result ledger</h3></div><span>{universe.applications.length}</span></header>{universe.applications.length === 0 ? <div className="empty-state compact">No standalone result has been applied yet.</div> : [...universe.applications].sort((left, right) => right.officialOrderKey.localeCompare(left.officialOrderKey)).map((application) => <button key={application.id} type="button" className={`${selectedApplication?.id === application.id ? "active" : ""} consequence-status--${statusClass(application.status)}`} onClick={() => setUniverse((current) => ({ ...current, settings: { ...current.settings, selectedApplicationId: application.id } }))}><strong>{resultHeadline(application.finalResult)}</strong><span>{application.showName} · #{application.runningOrderPosition + 1} {application.segmentTitle}</span><small>{application.status} · {application.replayStatus} · {application.officialShowDate || "Date unavailable"}</small></button>)}</section>
    ...
    {universe.settings.activeTab === "audit" && <div className="consequence-audit"><section><header><div><p className="eyebrow">ROLLBACK</p><h3>Exclude one event and replay everything after it</h3></div><span>{selectedApplication?.status ?? "None"}</span></header>{selectedApplication ? <><p>Rollback removes only this consequence event. Later records, rankings, profiles, title activity, and competition results remain intact and are replayed in official chronology.</p><label className="field"><span>Rollback reason</span><textarea aria-label="Consequence rollback reason" rows={3} value={rollbackReason} onChange={(event) => setRollbackReason(event.target.value)} /></label><button className="secondary-button" type="button" disabled={selectedApplication.status !== "Applied"} onClick={rollback}>Rollback and Replay Later Results</button></> : <p>Select an applied result from Overview.</p>}</section><section><header><div><p className="eyebrow">PERMANENT AUDIT</p><h3>Consequence decisions and corrections</h3></div><span>{universe.audit.length}</span></header>{universe.audit.map((entry) => <article key={entry.id}><strong>{entry.action}</strong><span>{entry.detail}</span><small>{formatDate(entry.createdAt)}</small></article>)}</section></div>}
  </section>;
}
