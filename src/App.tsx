...
type ViewName = "control" | "planner" | "storyline-hub" | "worker-hub" | "shows" | "tew-storylines" | "schema";
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div><span className="brand-kicker">WRESTLING SIM</span><h1>TEW IX Story Tracker</h1></div><div className="phase-badge">PHASE 3C · CREATIVE CONTROL</div></header>
    ...
    <main>
      ...
      {loading && !["control", "planner", "storyline-hub", "worker-hub"].includes(view) && <div className="status-banner" role="status">Reading the database and matching TEW history tables…</div>}
      {error && !["control", "planner", "storyline-hub", "worker-hub"].includes(view) && <div className="status-banner error" role="alert"><strong>Import failed</strong><span>{error}</span></div>}
    ...
    <footer>Creative plans, shows, storylines, and worker profiles are stored in this browser. TEW snapshot access remains read-only.</footer>
  </div>;
}
