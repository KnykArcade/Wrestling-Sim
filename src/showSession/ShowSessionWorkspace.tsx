        ...
        <main className="show-session-main">
          {activeStep === "overview" || !selectedSegment || !selectedSummary ? <>
            ...
            {activeStep === "creative" && <section className="session-creative-step">
              ...
              <section className="session-step-panel"><header><div><p className="eyebrow">APPLIED CREATIVE OUTPUT</p><h3>{selectedSegment.type === "match" ? "Match Story and key moments" : "Angle Segment Output"}</h3></div><span>{selectedSummary.outputComplete ? "Complete" : "Draft"}</span></header><label className="field field--full"><span>{selectedSegment.type === "match" ? "Match Story" : "Angle Segment Output"}</span><textarea aria-label="Session current output" rows={14} value={selectedSegment.type === "match" ? selectedSegment.matchStory : selectedSegment.segmentOutput} onChange={(event) => updateSegment(selectedSegment.type === "match" ? { ...selectedSegment, matchStory: event.target.value } : { ...selectedSegment, segmentOutput: event.target.value })} /></label>{selectedSegment.type === "match" && <label className="field field--full"><span>Key moments / road-agent phase map</span><textarea aria-label="Session key moments" rows={6} value={selectedSegment.keyMoments} onChange={(event) => updateSegment({ ...selectedSegment, keyMoments: event.target.value })} /></label>}<footer><button className="primary-button" type="button" onClick={markOutputApplied}>Mark Current Output Applied</button><button className="secondary-button" type="button" onClick={() => setStep("package")}>Review Production Package</button></footer></section>
            </section>}
