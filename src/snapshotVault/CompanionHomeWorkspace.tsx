...
export default function CompanionHomeWorkspace({
  ...
  function exportCompleteBackup(): void {
    ...
    downloadJson(`tew-story-tracker-backup-v21-${timestamp.slice(0, 10)}.json`, backup);
    updateVault({ ...vault, dataCenter: { ...vault.dataCenter, lastCompleteBackupAt: timestamp } });
    setNotice("Version 21 complete companion backup exported. Parsed Snapshot Vault contents remain in the separate vault package.");
  }
  ...
  return <section className="companion-home-workspace" aria-label="TEW Companion Home">
    <header className="companion-home-hero">
      <div><p className="eyebrow">PHASE 5J · TEW COMPANION HOME</p><h2>Your current TEW snapshot, current show, next action, and complete data safety in one place</h2><p>Parsed TEW history is preserved read-only in IndexedDB. Match approaches, Match Stories, Angle Outputs, card entry, reconciliation, and Post-Show Wrap-Up remain the companion's only daily responsibilities.</p></div>
    ...
    {vault.home.activeTab === "home" && <div className="companion-home-grid">
      <main className="companion-home-main">
        <section className="companion-home-current">
          ...
          <div className="companion-home-actions"><button className="primary-button" type="button" disabled={!currentShow} onClick={onContinueShow}>Continue Current Show</button><button className="secondary-button" type="button" onClick={() => mdbInputRef.current?.click()}>{snapshotLoading ? "Reading Snapshot…" : "Import Updated TEW Snapshot"}</button><button className="secondary-button" type="button" onClick={onOpenResults}>Review New TEW Results</button><button className="secondary-button" type="button" disabled={wrapUpPending.length === 0} onClick={onOpenWrapUp}>Finish Post-Show Wrap-Up</button><button className="secondary-button" type="button" onClick={onOpenCalendar}>Open Promotion Calendar</button><button className="secondary-button" type="button" onClick={exportCompleteBackup}>Export Complete Backup</button></div>
        </section>
      ...
      <aside className="companion-home-sidebar">
        ...
        <section><header><h3>Data safety</h3><span>v21</span></header><dl><div><dt>Last complete backup</dt><dd>{formatDate(vault.dataCenter.lastCompleteBackupAt)}</dd></div><div><dt>Snapshot package</dt><dd>{formatDate(vault.dataCenter.lastVaultExportAt)}</dd></div><div><dt>Vault storage</dt><dd>{estimate ? formatBytes(estimate.totalBytes) : "Calculating…"}</dd></div><div><dt>Stored snapshots</dt><dd>{vault.manifest.length}</dd></div></dl><button className="secondary-button" type="button" onClick={() => setTab("data")}>Open Data Center</button></section>
      </aside>
    </div>}
