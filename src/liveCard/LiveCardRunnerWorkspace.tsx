...
import { applyCoreResultConsequences, recordAngleCompetitiveAdjustment } from "../consequences/model";
...
import { applyAngleEvaluation, calculateAngleEvaluation, createShowExpectationSnapshot, evaluateCompletedShow, finalizeAngleEvaluation, isAngleEvaluationCurrent } from "../showEvaluation/model";
...
function resultHeadline(result: NonNullable<ReturnType<typeof activeResolutionAttempt>>["finalResult"]): string {
  if (!result) return "No official result yet";
...
function resultFacts(record: MatchResolutionRecord | null): string[] {
  ...
  return [
    `${resultHeadline(attempt.finalResult)}.`,
...
export default function LiveCardRunnerWorkspace({ onOpenResolution, onOpenConsequences, onOpenPlanner }: LiveCardRunnerWorkspaceProps) {
  ...
  const currentProfiles = loadMatchEngineUniverse(window.localStorage).profiles;
  ...
  const angleEvaluationStale = Boolean(currentAngleEvaluation && !currentAngleEvaluation.appliedAt && selectedShow && currentSegment && !isAngleEvaluationCurrent(currentAngleEvaluation, selectedShow, currentSegment, currentProfiles));
  ...
  useEffect(() => {
    ...
    if (currentSegment.type === "angle") {
      ...
      const noContest = currentAttempt?.finalResult?.finishType === "No Contest";
      const winner = currentAttempt?.finalResult?.winnerName ?? "Winner";
      setAngleTitle(noContest ? "No Contest Follow-Up" : `${winner} Post-Match Reaction`);
  ...
  async function startShow(): Promise<void> {
    ...
    const activeUniverse = await loadActiveStartingUniverse(loadStartingUniverseState(window.localStorage));
    const company = activeUniverse?.companies.find((item) => item.id === activeUniverse.playableCompanyId) ?? null;
    const snapshot = createShowExpectationSnapshot(evaluations, selectedShow, { company, profiles: currentProfiles });
    updateSession((value) => startLiveCardSession(value, snapshot.crowdStart, snapshot));
  ...
  function lockCurrentMatch(): void {
    ...
    try {
      const anticipation = currentResolution.setup.anticipation ?? { score: 50, label: "Interested" as const, popularity: 50, momentum: 50, skills: 50, styleAppeal: 50 };
      const crowdRatedResolution = finalizeResolutionForLiveCrowd(currentResolution, anticipation, session.currentCrowd);
      const nextSession = lockMatchResult(session, crowdRatedResolution, anticipation);
      setUniverse((current) => upsertLiveCardSession(current, nextSession));
      const nextResolutions = { ...resolutions, records: resolutions.records.map((record) => record.id === crowdRatedResolution.id ? crowdRatedResolution : record) };
      setResolutions(nextResolutions);
      saveMatchResolutionUniverse(window.localStorage, nextResolutions);
      const currentConsequences = loadResultConsequenceUniverse(window.localStorage);
      const attempt = activeResolutionAttempt(crowdRatedResolution);
      ...
      const consequenceResult = applyCoreResultConsequences({
        ...
        resolution: crowdRatedResolution,
      ...
      saveChampionshipUniverse(window.localStorage, consequenceResult.championships);
      saveCompetitionUniverse(window.localStorage, consequenceResult.competitions);
  ...
  function completeAngle(): void {
    ...
    try {
      ...
      const calculated = calculateAngleEvaluation(selectedShow!, currentSegment, currentProfiles, session.currentCrowd);
  ...
  function finalizeCurrentAngle(mode: "accept" | "override"): void {
    ...
    try {
      ...
      const finalized = finalizeAngleEvaluation(currentAngleEvaluation, mode === "override" ? angleOverrideScore : undefined, mode === "override" ? angleOverrideReason : "", { show: selectedShow!, segment: currentSegment, profiles: matchEngine.profiles });
      const applied = applyAngleEvaluation(evaluations, finalized, matchEngine.profiles);
      const appliedEvaluation = applied.universe.angleEvaluations.find((item) => item.id === finalized.id)!;
      const plannedShows = loadPlannedShows(window.localStorage);
      const competitive = recordAngleCompetitiveAdjustment({
        universe: loadResultConsequenceUniverse(window.localStorage),
        evaluation: appliedEvaluation,
        show: selectedShow!,
        profilesBefore: matchEngine.profiles,
        profilesAfter: applied.profiles,
        shows: plannedShows,
        championships: loadChampionshipUniverse(window.localStorage),
        competitions: loadCompetitionUniverse(window.localStorage),
      });
      saveMatchEngineUniverse(window.localStorage, { profiles: competitive.profiles });
      saveResultConsequenceUniverse(window.localStorage, competitive.universe);
      const next = completeAngleSegment(session, currentSegment.id, currentSegment.segmentOutput, currentSegment.consequences, currentSegment.followUp, finalized.rawPerformance, finalized.anticipation, finalized.finalScore, finalized.calculationLedger?.finalRating);
      setEvaluations(applied.universe);
      setConsequences(competitive.universe);
      setUniverse((current) => upsertLiveCardSession(current, next));
  ...
  return <section className="live-card-workspace">
    ...
    {!selectedShow || !session ? <div className="empty-state live-card-empty"><h3>No planned show is available</h3><p>Create the card first. Participants and context may be booked, but match winners can remain unresolved.</p></div> : <>
      ...
      <section className="live-card-summary"><article><span>Show status</span><strong>{session.status}</strong></article><article><span>Card progress</span><strong>{completedSegments}/{session.progress.length}</strong></article><article><span>Remaining</span><strong>{remainingSegments}</strong></article><article><span>Crowd heat</span><strong>{session.currentCrowd.toFixed(1)}</strong><small>Started {session.crowdStart.toFixed(1)}</small></article><article><span>Current segment</span><strong>{currentSegment ? session.segmentOrder.indexOf(currentSegment.id) + 1 : 0}</strong></article></section>
      ...
      <div className="live-card-layout"><aside className="live-card-running-order"><header><div><p className="eyebrow">RUNNING ORDER</p><h3>{selectedShow.name}</h3><span>{selectedShow.date || "Unscheduled"}</span></div><b>{session.progress.length}</b></header><div>{session.segmentOrder.map((segmentId, index) => { const progress = session.progress.find((item) => item.segmentId === segmentId); const segment = selectedShow.segments.find((item) => item.id === segmentId); if (!progress || !segment) return null; return <button type="button" key={segmentId} className={`${currentSegment?.id === segmentId ? "active" : ""} live-card-status--${statusClass(progress.status)}`} onClick={() => selectSegment(segmentId)}><strong>{index + 1}</strong><span><b>{segment.title}</b><small>{segment.type === "match" ? "Match" : "Angle"} · {segment.durationMinutes} min</small><em>{progress.status}{progress.insertedDuringShow ? " · Inserted live" : ""}</em></span></button>; })}</div><footer><button className="secondary-button" type="button" onClick={() => moveNext("segment")}>Next Unfinished Segment</button><button className="secondary-button" type="button" onClick={() => moveNext("match")}>Next Match</button></footer></aside>
        ...
        <main className="live-card-current">{!currentSegment || !currentProgress ? <div className="empty-state"><h3>No current segment</h3></div> : <>
          ...
          {currentSegment.type === "match" ? <section className="live-card-match-panel"><header><div><p className="eyebrow">OFFICIAL MATCH RESULT</p><h3>{resultReady ? resultHeadline(currentAttempt?.finalResult ?? null) : currentResolution ? "Engine result awaiting acceptance or override" : "No official result yet"}</h3></div><span>{currentResolution?.status ?? "Unresolved"}</span></header>{currentAttempt ? <div className="live-card-result-facts">{resultFacts(currentResolution).map((fact) => <p key={fact}>{fact}</p>)}</div> : <p>Open Run Matches. The wrestlers choose approaches and the engine produces one official result for the current setup.</p>}{currentProgress.audience && <div className="live-card-audience-result" aria-label="Official live crowd result"><strong>Performance {currentProgress.audience.performanceRating.toFixed(1)} · Crowd Response {currentProgress.audience.crowdResponse.toFixed(1)} · Final Rating {currentProgress.audience.finalRating.toFixed(1)}</strong><span>Crowd Heat {currentProgress.audience.crowdBefore.toFixed(1)} → {currentProgress.audience.crowdAfter.toFixed(1)} · Anticipation {currentProgress.audience.anticipation.toFixed(1)} {currentProgress.audience.anticipationLabel}</span></div>}<div className="live-card-actions"><button className="primary-button" type="button" onClick={openResolution}>{currentResolution ? "Open Official Match Result" : "Run This Match"}</button><button className="secondary-button" type="button" disabled={!resultReady || currentProgress.status === "Completed"} onClick={lockCurrentMatch}>Lock Result Into Live Card</button></div>
            ...
            {currentAngleEvaluation && !currentAngleEvaluation.appliedAt && currentProgress.status !== "Correction" && <section className="live-card-angle-result-review" aria-label="Angle result review">
              <header><div><p className="eyebrow">RESULT REVIEW</p><h3>{currentAngleEvaluation.calculatedScore.toFixed(1)} official angle rating</h3><p>Nothing changes permanently until you accept this result or explain an override.</p></div><strong>{currentAngleEvaluation.calculatedScore.toFixed(1)}</strong></header>
              {angleEvaluationStale && <div className="live-card-readiness" role="alert"><p>This calculation is stale because the angle, participants, roles, show placement, or a participant profile changed. Recalculate before accepting it.</p></div>}
              <div className="live-card-evaluation-factors">
                <article><span>Raw performance</span><b>{currentAngleEvaluation.rawPerformance.toFixed(1)}</b><small>Participant execution and angle structure only</small></article>
                <article><span>Anticipation</span><b>{currentAngleEvaluation.anticipation.toFixed(1)}</b><small>Popularity, momentum, fan presentation, and stakes</small></article>
                <article><span>Live crowd response</span><b>{currentAngleEvaluation.crowdResponse.toFixed(1)}</b><small>Incoming heat {currentAngleEvaluation.crowdBefore.toFixed(1)}</small></article>
                <article><span>Official rating</span><b>{currentAngleEvaluation.calculatedScore.toFixed(1)}</b><small>60% raw performance and 40% crowd response</small></article>
              </div>
              <div className="live-card-participant-results">{currentAngleEvaluation.participants.map((participant) => <article key={participant.workerKey}><div><strong>{participant.workerName}</strong><span>{participant.role}</span></div><b>{participant.performanceScore.toFixed(1)}</b><small>Momentum {participant.momentumBefore.toFixed(1)} → {participant.momentumAfter.toFixed(1)} · Popularity {participant.popularityBefore.toFixed(1)} → {participant.popularityAfter.toFixed(1)}</small>{participant.calculationLedger && <details className="calculation-ledger-details"><summary>Open participant calculation</summary><div className="calculation-stage-stack"><CalculationStage stage={participant.calculationLedger.rolePerformance} /><CalculationStage stage={participant.calculationLedger.creditedPerformance} /><CalculationStage stage={participant.calculationLedger.momentum} /><CalculationStage stage={participant.calculationLedger.popularity} /></div></details>}</article>)}</div>
              {currentAngleEvaluation.calculationLedger && <details className="calculation-ledger-details"><summary>Open exact angle calculation · {currentAngleEvaluation.calculationVersion}</summary><div className="calculation-stage-stack"><CalculationStage stage={currentAngleEvaluation.calculationLedger.participantExecution} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.structure} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.rawPerformance} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.popularity} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.momentum} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.fanPresentation} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.storyStakes} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.anticipation} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.expectationAdjustment} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.crowdResponse} /><CalculationStage stage={currentAngleEvaluation.calculationLedger.finalRating} /></div></details>}
              <div className="live-card-actions"><button className="primary-button" type="button" disabled={angleEvaluationStale} onClick={() => finalizeCurrentAngle("accept")}>Accept Angle Result</button></div>
              <div className="live-card-angle-override"><label className="field"><span>Override rating</span><input aria-label="Angle override rating" type="number" min="0" max="100" value={angleOverrideScore} onChange={(event) => setAngleOverrideScore(Number(event.target.value))} /></label><label className="field"><span>Override reason</span><textarea aria-label="Angle override reason" rows={3} value={angleOverrideReason} onChange={(event) => setAngleOverrideReason(event.target.value)} /></label><button className="secondary-button" type="button" disabled={angleEvaluationStale} onClick={() => finalizeCurrentAngle("override")}>Use Explained Override</button></div>
            </section>}
            {currentProgress.status === "Completed" && currentAngleEvaluation?.appliedAt && <><div className="live-card-audience-result" aria-label="Official angle rating breakdown"><strong>Raw Performance {currentAngleEvaluation.rawPerformance.toFixed(1)} · Crowd Response {currentAngleEvaluation.crowdResponse.toFixed(1)} · Official Rating {currentAngleEvaluation.finalScore.toFixed(1)}</strong><span>Incoming Heat {currentAngleEvaluation.crowdBefore.toFixed(1)} · Anticipation {currentAngleEvaluation.anticipation.toFixed(1)}</span></div><section className="live-card-next-step"><div><p className="eyebrow">ANGLE CONSEQUENCES APPLIED</p><h3>{currentAngleEvaluation.finalScore.toFixed(1)} · {currentAngleEvaluation.status}</h3><p>Participant momentum and popularity effects were recorded exactly once.</p></div><button className="primary-button" type="button" onClick={() => moveNext("segment")}>Continue to Next Segment</button></section></>}{currentProgress.status === "Completed" && <div className="live-card-correction"><label className="field"><span>Correction reason</span><textarea aria-label="Live card correction reason" rows={3} value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} /></label><button className="secondary-button" type="button" onClick={openCorrection}>Open Explicit Correction</button></div>}{currentProgress.corrections.length > 0 && <details className="live-card-correction-history"><summary>Correction history</summary>{currentProgress.corrections.map((entry) => <article key={entry.id}><strong>{entry.reason}</strong><span>{entry.beforeOutput || "No prior output"}</span><b>→</b><span>{entry.afterOutput || "Correction still open"}</span><small>{formatDate(entry.openedAt)}{entry.completedAt ? ` · completed ${formatDate(entry.completedAt)}` : ""}</small></article>)}</details>}</section>}
      ...
      {session.status === "Completed" && <section className="live-card-complete-summary"><header><div><p className="eyebrow">POST-SHOW REPORT</p><h3>{selectedShow.name} is complete</h3><p>{completedSegments} completed · {session.progress.filter((item) => item.status === "Skipped").length} skipped · {insertedSegments} inserted live</p></div><span>{formatDate(session.completedAt)}</span></header>{showReport && <><div className="live-card-show-score"><article><span>Overall rating</span><strong>{showReport.overallScore.toFixed(1)}</strong><small>{showReport.audienceReaction} · expected {showReport.expectedShowScore.toFixed(1)}</small></article><article><span>Attendance</span><strong>{showReport.estimatedAttendance.toLocaleString()}</strong><small>{showReport.attendanceCalculation.capacityLimited ? `Capacity-limited from ${showReport.attendanceCalculation.unconstrainedDemand.toLocaleString()} frozen demand` : "Frozen when the show started"}</small></article><article><span>Crowd finish</span><strong>{showReport.crowdFinish.toFixed(1)}</strong><small>Started at {showReport.crowdStart.toFixed(1)}</small></article><article><span>Promotion popularity</span><strong>{showReport.promotionPopularityAfter.toFixed(1)}</strong><small>{showReport.promotionPopularityBefore.toFixed(1)} {showReport.promotionPopularityDelta >= 0 ? "+" : ""}{showReport.promotionPopularityDelta.toFixed(1)}</small></article></div><div className="live-card-crowd-path">{showReport.segments.map((segment, index) => <article key={segment.segmentId}><b>{index + 1}</b><div><strong>{segment.segmentTitle}</strong><span>{segment.segmentType === "match" ? "Match" : "Angle"} · {segment.durationMinutes.toFixed(1)} min · {segment.reaction} · section {segment.sectionWeight.toFixed(2)} × duration {segment.durationWeight.toFixed(2)} × main event {segment.mainEventWeight.toFixed(2)} = {segment.importanceWeight.toFixed(2)}</span></div><em>{segment.receptionScore.toFixed(1)}</em><small>Raw {segment.score.toFixed(1)} · audience {segment.crowdModifier >= 0 ? "+" : ""}{segment.crowdModifier.toFixed(1)} · weighted contribution {segment.weightedContribution.toFixed(2)} · heat {segment.crowdBefore.toFixed(1)} → {segment.crowdAfter.toFixed(1)}</small></article>)}</div><details className="live-card-report-explanation"><summary>How this show was evaluated</summary>{showReport.explanations.map((explanation) => <p key={explanation}>{explanation}</p>)}</details>{showReport.calculationLedger && <details className="calculation-ledger-details"><summary>Open exact show calculation · {showReport.calculationVersion}</summary><div className="calculation-stage-stack"><CalculationStage stage={showReport.calculationLedger.promotionStrength} /><CalculationStage stage={showReport.calculationLedger.expectedCardStrength} /><CalculationStage stage={showReport.calculationLedger.startingCrowd} /><CalculationStage stage={showReport.calculationLedger.expectedShowScore} /><CalculationStage stage={showReport.calculationLedger.attendanceDemand} /><CalculationStage stage={showReport.calculationLedger.overallScore} /><CalculationStage stage={showReport.calculationLedger.promotionPopularity} /></div></details>}</>}{session.progress.map((progress, index) => <article key={progress.segmentId}><b>{index + 1}</b><div><strong>{progress.title}</strong><span>{progress.result ? resultHeadline(progress.result.finalResult) : progress.finalAngleOutput || progress.skipReason || progress.status}</span></div><em>{progress.status}</em></article>)}</section>}
