...
import { applyAngleEvaluation, calculateAngleEvaluation, evaluateCompletedShow, finalizeAngleEvaluation, startingCrowdForShow } from "../showEvaluation/model";
...
export default function LiveCardRunnerWorkspace({ onOpenResolution, onOpenConsequences, onOpenPlanner }: LiveCardRunnerWorkspaceProps) {
  ...
  async function startShow(): Promise<void> {
    ...
    const crowdStart = startingCrowdForShow(selectedShow, company, evaluations.promotionPopularity, evaluations.promotionPopularitySeeded);
    updateSession((value) => startLiveCardSession(value, crowdStart));
  ...
  function finalizeCurrentAngle(mode: "accept" | "override"): void {
    ...
    try {
      ...
      const next = completeAngleSegment(session, currentSegment.id, currentSegment.segmentOutput, currentSegment.consequences, currentSegment.followUp, finalized.finalScore);
