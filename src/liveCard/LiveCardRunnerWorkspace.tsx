...
export default function LiveCardRunnerWorkspace({ onOpenResolution, onOpenConsequences, onOpenPlanner }: LiveCardRunnerWorkspaceProps) {
  ...
  function completeShow(): void {
    ...
    try {
      ...
      const evaluated = evaluateCompletedShow(evaluations, selectedShow, next);
      setUniverse((current) => upsertLiveCardSession(current, next));
  ...
  return <section className="live-card-workspace">
    ...
    {!selectedShow || !session ? <div className="empty-state live-card-empty"><h3>No planned show is available</h3><p>Create the card first. Participants and context may be booked, but match winners can remain unresolved.</p></div> : <>
      ...
      {session.status === "Completed" && <section className="live-card-complete-summary"><header><div><p className="eyebrow">POST-SHOW REPORT</p><h3>{selectedShow.name} is complete</h3><p>{completedSegments} completed · {session.progress.filter((item) => item.status === "Skipped").length} skipped · {insertedSegments} inserted live</p></div><span>{formatDate(session.completedAt)}</span></header>{showReport && <><div className="live-card-show-score"><article><span>Overall rating</span><strong>{showReport.overallScore.toFixed(1)}</strong><small>{showReport.audienceReaction} audience reaction</small></article><article><span>Attendance</span><strong>{showReport.estimatedAttendance.toLocaleString()}</strong><small>In-game estimate</small></article><article><span>Crowd finish</span><strong>{showReport.crowdFinish.toFixed(1)}</strong><small>Started at {showReport.crowdStart.toFixed(1)}</small></article><article><span>Promotion popularity</span><strong>{showReport.promotionPopularityAfter.toFixed(1)}</strong><small>{showReport.promotionPopularityBefore.toFixed(1)} {showReport.promotionPopularityDelta >= 0 ? "+" : ""}{showReport.promotionPopularityDelta.toFixed(1)}</small></article></div><div className="live-card-crowd-path">{showReport.segments.map((segment, index) => <article key={segment.segmentId}><b>{index + 1}</b><div><strong>{segment.segmentTitle}</strong><span>{segment.segmentType === "match" ? "Match" : "Angle"} · {segment.reaction} · weight {segment.importanceWeight.toFixed(2)}×</span></div><em>{segment.score.toFixed(1)}</em><small>Crowd {segment.crowdBefore.toFixed(1)} → {segment.crowdAfter.toFixed(1)}</small></article>)}</div><details className="live-card-report-explanation"><summary>How this show was evaluated</summary>{showReport.explanations.map((explanation) => <p key={explanation}>{explanation}</p>)}</details></>}{session.progress.map((progress, index) => <article key={progress.segmentId}><b>{index + 1}</b><div><strong>{progress.title}</strong><span>{progress.result ? `${progress.result.finalResult.winnerName} defeated ${progress.result.finalResult.loserName}` : progress.finalAngleOutput || progress.skipReason || progress.status}</span></div><em>{progress.status}</em></article>)}</section>}
