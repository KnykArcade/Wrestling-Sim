...
test("calculates and accepts an angle before producing one permanent post-show report", async ({ page }) => {
  ...
  await expect(page.getByRole("table", { name: "Raw angle performance exact inputs" })).toBeVisible();
  await expect(page.getByRole("table", { name: "Angle anticipation exact inputs" })).toBeVisible();
